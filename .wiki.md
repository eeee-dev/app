# Project Summary
The Financial Management Dashboard for the ë Ecosystem is an advanced platform aimed at optimizing financial operations for businesses. It offers tools for budget management, expense tracking, income reporting, and invoice processing, ensuring compliance with regulations such as Mauritius VAT. The platform utilizes automation and analytics to improve operational efficiency, enabling users to make informed financial decisions with a focus on accessibility and ease of deployment. Recent updates include a flexible project-department structure and enhancements to the Income Category Breakdown System.
...
3. **Run SQL Migration**: 
   - Open the Supabase SQL Editor and run the migration scripts located at `supabase-migrations/migration-project-departments.sql` and `supabase-migrations/migration-income-categories.sql`.
4. **Run Setup Script**: 
   ```bash
