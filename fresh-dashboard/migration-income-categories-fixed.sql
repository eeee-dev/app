-- Income Category Breakdown System Migration (FIXED VERSION)
-- This migration adds support for breaking down income/invoices into multiple categories
-- Fixed: Handles missing departments table gracefully
...
-- Add department_id column ONLY if departments table exists
DO $$ 
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'app_72505145eb_departments') THEN
